'''Performs SVR and Decision Tree regression for all batches from a dataset.

   Author: Jonathan Paynter
   Date: 11/11/2013

   Usage: python trees-svr-compare.py

   This script applies decision trees and SVR to learn models for all batch
   values for a single dataset (one assembly, reaction rate, energy). The data 
   is stored as 18-dimensional feature vectors for each sample with a single 
   target value. The feature descriptions are given in process/samples.py. The 
   target values for each sample are the converged (after 1000 batches) pinwise 
   tallies (across the entire 3x3 mesh) for each reaction rate and energy.

   This script splits the samples into training and test sets. The next step
   is to train scikit-learn's decision tree and SVR models using the training 
   data. Finally, the models are used to predict the target values for the test 
   samples. The Root Mean Square (RMS) error with respect to the true target 
   values is computed and reported to the bash for each model. Plots of the RMS
   error for each model at each batch are generated and displayed.
'''
...
for batch in Batches:
    ...
    #### Fit decision tree regression models
    ...
    for i in range(testing):
        ...
        error_test_tree1[count] = error_test_tree1[count] + \
                                  diff_test_tree1*diff_test_tree1
        error_test_tree2[count] = error_test_tree2[count] + \
                                  diff_test_tree2*diff_test_tree2
        ...
        error_test_rbf[count] = error_test_rbf[count] + \
                                diff_test_rbf*diff_test_rbf
        error_test_lin[count] = error_test_lin[count] + \
                                diff_test_lin*diff_test_lin
        error_test_poly[count] = error_test_poly[count] + \
                                diff_test_poly*diff_test_poly
...
for batch in Batches:
    ...
    print batch
    print 'For ' + batch + ', Regression tree 1 testing error is %f' % \
        (error_test_tree1[count])
    print 'For ' + batch + ', Regression tree 2 testing error is %f' % \
        (error_test_tree2[count])
    print 'For ' + batch + ', RBF kernel testing error is %f' % \
        (error_test_rbf[count])
    print 'For ' + batch + ', Linear kernel testing error is %f' % \
        (error_test_lin[count])
    print 'For ' + batch + ', Polynomial, 2nd degree, testing error is %f' % \
        (error_test_poly[count])
